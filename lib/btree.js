...
// Split the `splitChild` node in the middle.
...
//
// i: index of splitChild in this.children.
Node.prototype.split = function(i, splitChild) {
  ...
  newChild.leaf = splitChild.leaf

  // Copy over the "right half" of y into z
  newChild.keys = splitChild.keys.slice(middle + 1)
  newChild.values = splitChild.values.slice(middle + 1)

  // Copy over the children if split child isn't a leaf
	if (!splitChild.leaf) {
